<template>
  <div>
    ...
    <el-dialog title="评估" :visible.sync="dialogTableVisible" >
      ...
      <div v-if="key!='No'&& key!='Score' && key!='context' && key!='score'" style="margin: 20px 0px;font-weight:bold" v-for="(val, key, index) in evaluatedData" :key="v0">
    ...
    rowSpan(spanArr, position, spanName) {
      this.tableData.forEach((item, index) =>{
          if (v0 === 0){
            ...
            if (this.tableData[v0][spanName] === this.tableData[v0 - 1][spanName]){
              spanArr[position] += 1;
              spanArr.push(0);
            }else{
              spanArr.push(1);
