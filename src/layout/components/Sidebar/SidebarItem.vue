<template>
  <div v-if="!item.hidden">
    <template v-if="hasOneShowingChild(item.children, item) && (!onlyOneChild.children || onlyOneChild.noShowingChildren) && !item.alwaysShow">
      <el-tooltip class="item" effect="dark" :content="generateTitle(onlyOneChild.meta.title)" placement="right">
    ...
    <el-submenu v-else ref="subMenu" :index="resolvePath(item.path)" popper-append-to-body>
      <template slot="title">
        <el-tooltip class="item" effect="dark" :content="generateTitle(item.meta.title)" placement="right">

          <!--将原来的普通标签改为路由标签，样式的设定与./Item.vue相同-->
          <router-link :to="resolvePath(item.path)">
            <!-- <el-menu-item :class="{'submenu-title-noDropdown':!isNest}"> -->
            <i :class="item.meta.icon"
              style="color: currentColor;width: 1em;height: 1em;margin-right: 12px;margin-left: -2px"></i>
